...
const TOKEN_ADDRESS_BY_SYMBOL = {
  ...
  QDRC: "0xBE7477BF91526FC9988C8f33e91B6db687119D45",
};
...
function resolvePoolAddressForToken(tokenAddressInput) {
  ...
  const qdrcToken = ethers.getAddress(TOKEN_ADDRESS_BY_SYMBOL.QDRC);
  ...
  if (tokenAddress === qdrcToken) {
    return String(
      process.env.PRIVACY_POOL_ADDRESS_QDRC || process.env.VITE_PRIVACY_POOL_ADDRESS_QDRC || ""
    ).trim();
  }
  return "";
}
...
export async function executeRecurringPrivpayDeposit(schedule) {
  ...
  if (!poolAddress.startsWith("0x")) {
    throw new Error(
      `Server missing privacy pool mapping for token ${schedule.tokenAddress}. Set PRIVACY_POOL_ADDRESS_USDC/EURC/QDRC.`
    );
  }
